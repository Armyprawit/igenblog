...
// scroll event to load more article
$(window).scroll(function() {
  if($(window).scrollTop() + $(window).height() >= $(document).height()-50) {
  	if(state){
  		loadingArticle($('#categoryMode').val(),index);
  	}
  }
});

// Crete new Article
function createArticle(image,title,context,category,keyword,credit){
	state = true;
	showLiveView('close');
	...
	var url = 'process/process-article-create.php';
	context = context.replace(/\r?\n/g,"<br>");
	var pmeters = 'image='+image+'&title='+title+'&context='+context+'&category='+category+'&keyword='+keyword+'&credit='+credit;
...
function editArticle(id,image,title,context,category,keyword,credit){
	state = true;
	showLiveView('close');
	$("#console").animate({bottom:"0px"},800);
...
function statusArticle(id,stat){
	...
	var url = 'process/process-article-status.php';
...
function modeListArticle(category){
	state = true;
	$("#list").fadeOut(100);
	...
	var url = 'process/mode-article.php';
...
function searchArticle(q){
	...
	var url = 'process/process-article-search.php';
	var pmeters = 'q='+q;
...
function loadingArticle(category,start){
	state = true;
	loading = false;
	...
	var url = 'process/process-article-loading.php';
...
function toSelectArticle(id){
	...
	var url = 'process/edit-article.php';
	var pmeters = 'id='+id;
...
function toCreateArticle(){
	...
	var url = 'process/process-article-createform.php';
	var pmeters = 'id='+0;

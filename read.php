...
<?php
  ...
  $_GET['c'] = $articleData['ca_url'];
  $article->updateStatus($dbHandle,'view',$articleData['ar_id']);
?>
<!doctype html>
<html>
<head>
...
<!-- Meta Tag Main -->
<meta name="description"          content="<?php echo $mydev->convTextToMetaTag($articleData['ar_description']);?>">
<meta name="keywords"             content="<?php echo $articleData['ar_keyword'];?>" />

<meta property="og:title"         content="<?php echo $mydev->convTextToMetaTag($articleData['ar_title']);?> : <?php echo $setting->getSetting($dbHandle,1);?>"/>
<meta property="og:description"   content="<?php echo $mydev->convTextToMetaTag($articleData['ar_description']);?>"/>
<meta property="og:url"           content="<?php echo $setting->getSetting($dbHandle,2);?>/read-<?php echo $articleData['ar_id']+1024;?>-<? echo $mydev->urlSEO($articleData['ar_title']);?>.html"/>
<meta property="og:image"         content="<?php echo $articleData['ar_image'];?>"/>
...
<meta itemprop="name"             content="<?php echo $mydev->convTextToMetaTag($articleData['ar_title']);?> : <?php echo $setting->getSetting($dbHandle,1);?>">
<meta itemprop="description"      content="<?php echo $mydev->convTextToMetaTag($articleData['ar_description']);?>">
<meta itemprop="image"            content="<?php echo $articleData['ar_image'];?>">

<title><?php echo $mydev->convTextToMetaTag($articleData['ar_title']);?> : <?php echo $setting->getSetting($dbHandle,1);?></title>
...
<div id="fb-root"></div>
<input type="hidden" id="articleID" value="<?php echo $articleData['ar_id'];?>">
...
<div id="mainContent">
  <div class="mainContent">
    ...
    <div id="show">
      <h1><?php echo stripslashes($articleData['ar_title']);?></h1>
      <div class="stat">
        <div class="time"><i class="fa fa-clock-o"></i> <?php echo $mydev->fb_thaidate($articleData['ar_post_time']);?></div>
        <div class="time"><i class="fa fa-folder"></i> <a href="cat-<?php echo $articleData['ca_url'];?>.html" target="_parent"><?php echo $articleData['ca_title'];?></a></div>
        <div class="statBox"><i class="fa fa-coffee"></i> <?php echo $articleData['ar_c_view'];?> Read</div>
      </div>

      <div class="photo"><img src="<?php echo $articleData['ar_image'];?>" alt=""></div>

      <div class="text"><?php echo stripslashes($articleData['ar_text']);?></div>

      <?php include'html/article-share.php';?>
...
<div id="feature">
  ...
  <?php $timeline->getFeedTimeline($dbHandle,'feature','normal',2,$articleData['ca_id'],0,7);?>
...
<script type="text/javascript">
  $(document).ready(function(){
    //setTimeout('updateWatch("article",$("#articleID").val());',12000);
  });
</script>
...
<?php
  ...
  $analytic->updatePageview($dbHandle,6);
